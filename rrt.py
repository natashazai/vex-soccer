...
from .worldmap import BarrelObj, SportsBallObj, AprilTagObj, ArucoMarkerObj, WallObj, DoorwayObj, RoomObj, RobotObj, WorldObject
...
class RRT():
    ...
    def generate_obstacles(self, goal_object,
        ...
        for obj in self.robot.world_map.objects.values():
            ...
            if isinstance(obj, BarrelObj):
                ...
                obst = None
            elif isinstance(obj, RobotObj):
                obst = self.generate_robot_obstacle(obj, obstacle_inflation)
            else:
    ...
    @staticmethod
    def generate_robot_obstacle(robot, inflation=0):
        s = Circle(center=geometry.point(robot.pose.x, robot.pose.y),
                   radius=35 + inflation)
        s.obstacle_id = robot.id
        return s
